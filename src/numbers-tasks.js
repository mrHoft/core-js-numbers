...
function getFibonacciNumber(index) {
  let fibo = [0, 1];
  for (let i = 1; i < index; i += 1) fibo = [fibo[1], fibo[1] + fibo[0]];
  return index <= 1 ? index : fibo[1];
}
// Array:
function getFibonacciNumber(index) {
  const fibo = [0, 1];
  for (let i = 1; i < index; i += 1) fibo.push(fibo[i] + fibo[i - 1]);
  return index <= 1 ? index : fibo.slice(-1);
}
// Recursive:
function getFibonacciNumber(index) {
